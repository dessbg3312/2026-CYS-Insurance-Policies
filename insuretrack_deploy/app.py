...
def page_dashboard(data):
    ...
    # KPIs — white cards, colored numbers
    quote_n = sum(1 for p in props if p.get("coverage_status") == "Quote")
    kpis = [
        (str(len(props)),     "Total Properties", R['text_d']),
        (str(active_n),       "Insured Active",   "#16a34a"),
        (str(uninsu_n),       "Uninsured",        "#dc2626"),
        (str(verify_n + quote_n), "Verify / Quote", "#d97706"),
        (f"${prem_tot:,.0f}", "Annual Premium",   R['rose']),
        (str(tot_units),      "Total Units",      R['plum']),
    ]
    cols = st.columns(6)
